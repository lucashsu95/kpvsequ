...
<body>
  ...
  $sql = "select * from genrepair";
  if(@$_GET['key'] <> '' || @$_SESSION['user'] === 'repair'){
    $sql .= ' where processing="未處理"';
  }
  $sql .= " order by id desc limit $num,10";
  $query = $db->query($sql)->fetchAll();
  $line = count($db->query('select * from genrepair')->fetchAll());
  
  ?>
  <a href="genrepairlist.php?key=未處理" class='backBtn'>未處理</a>
  ...
  <div class="table">
    ...
    <?php foreach($query as $data){ ?>
      <div class="tr">
        ...
        <div><?php echo $data['genrepair_name'] ?></div>
        <div><?php echo $data['genrepair_place'] ?></div>
        <?php if(@$_SESSION['user'] === 'genrepair'){ ?>
        <form action="update.php?do=processing" method="post" id='processing<?php echo $data['id']?>'>
            ...
            <?php } ?>
        <div class='date'><?php echo $data['genrepair_date'] ?></div>
      </div>
    ...
    <section class='select_page'>
      ...
      <article>
          <?php if($num > 10){ ?>
              <a href='genrepairlist.php?num=0'>第一頁</a>
              <a href='genrepairlist.php?num=<?php echo $num - 10 ?>'>上一頁</a>
            <?php }else{ ?>
              <div></div>
              <div></div>
              <?php }
              ?>
              <a href='genrepairlist.php?num=<?php echo $num + 10 ?>'>下一頁</a>
              <a href='genrepairlist.php?num=<?php echo $line - ($line % 10) ?>'>最後一頁</a>
      </article>
    </section>
    <section>
    <?php if(@$_SESSION['user'] === 'genrepair'){ ?>
